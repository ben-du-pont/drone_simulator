...
from online_uwb_initialisation.core.uwb_online_initialisation import UwbOnlineInitialisation
...
class UwbOnlineInitialisationNode(Node):
    ...
    def _initialization_timer_callback(self):
        ...
        if not self.initialized and len(self.waypoints) > 0:
            ...
            self.uwb_online_initialisation.passed_waypoints = [self.waypoints[0]]
            self.uwb_online_initialisation.remaining_waypoints = self.waypoints[1:]
    ...
    def _waypoint_lists_callback(self, msg):
        ...
        # Update waypoints in the estimator
        self.uwb_online_initialisation.passed_waypoints = reached_waypoints
        self.uwb_online_initialisation.remaining_waypoints = remaining_waypoints

        # # checkpoint logging
        # self.get_logger().info(f"Passed waypoints: {self.uwb_online_initialisation.passed_waypoints}")
        # self.get_logger().info(f"Length of passed waypoints: {len(self.uwb_online_initialisation.passed_waypoints)}")
        # self.get_logger().info(f"Remaining waypoints: {self.uwb_online_initialisation.remaining_waypoints}")
        # self.get_logger().info(f"Length of remaining waypoints: {len(self.uwb_online_initialisation.remaining_waypoints)}")

        # Check if the last passed waypoint matches the next optimal waypoint
        if self.uwb_online_initialisation.current_optimal_waypoints:
            if np.linalg.norm(np.array(self.uwb_online_initialisation.passed_waypoints[-1]) - 
                            np.array(self.uwb_online_initialisation.current_optimal_waypoints[0])) < 0.01:
                self.uwb_online_initialisation.current_optimal_waypoints = self.uwb_online_initialisation.current_optimal_waypoints[1:]
    ...
    def _drone_position_callback(self, msg):
        ...
        self.uwb_online_initialisation.drone_position = drone_position
        ...
        for anchor_id in self.unknown_anchors:
            ...
            if distance is not None:
                self.uwb_online_initialisation.measurement_callback(drone_position, distance, anchor_id)
        
        # Check for anchor status changes and publish estimates
        for anchor_id in self.unknown_anchors:
            if anchor_id in self.uwb_online_initialisation.anchor_measurements_dictionary:
                measurements_dict = self.uwb_online_initialisation.anchor_measurements_dictionary[anchor_id]
                current_status = measurements_dict.get("status", "unseen")
                ...
                self._publish_anchor_estimates(anchor_id, measurements_dict)
                
                # Check if we have a new optimized trajectory and need to publish it
                if (self.uwb_online_initialisation.status == "on_optimal_trajectory" and 
                    anchor_id not in self.published_optimized_trajectories and
                    len(self.uwb_online_initialisation.current_optimal_waypoints) > 0):
    ...
    def _anchor_info_callback(self, msg):
        ...
        """
        #self.get_logger().info(f"Received information for {msg.count} anchors")
        ...
        #self.get_logger().info(f"Processed {len(self.base_anchors)} known anchors and {len(self.unknown_anchors)} unknown anchors")


    def _publish_anchor_estimates(self, anchor_id, measurements_dict):
        """Publish various types of anchor estimates.
        
        Args:
            anchor_id: ID of the anchor
            measurements_dict: Dictionary with measurement and estimation data
        """
        # Publish linear estimate if available
        if ("estimator_rough_linear" in measurements_dict and 
            len(measurements_dict["estimator_rough_linear"]) >= 5 and 
            any(abs(x) > 0.0 for x in measurements_dict["estimator_rough_linear"])):
            estimate = measurements_dict["estimator_rough_linear"]
            position = estimate[:3]
        ...
        if "estimator_rough_non_linear" in measurements_dict and len(measurements_dict["estimator_rough_non_linear"]) >= 5 and measurements_dict["status"] == "on_optimal_trajectory":
            estimate = measurements_dict["estimator_rough_non_linear"]
            position = estimate[:3]
            constant_bias = estimate[3]
            linear_bias = estimate[4] if len(estimate) > 4 else 1.0
        ...
        if "estimator" in measurements_dict and len(measurements_dict["estimator"]) >= 5 and measurements_dict["status"] == "initialised":
            estimate = measurements_dict["estimator"]
            position = estimate[:3]
            constant_bias = estimate[3]
            linear_bias = estimate[4] if len(estimate) > 4 else 1.0
    ...
    def _publish_optimized_trajectory(self, anchor_id):
        ...
        """
        if len(self.uwb_online_initialisation.current_optimal_waypoints) == 0:
            return
        ...
        msg.header.frame_id = self.get_parameter('frame_id').get_parameter_value().string_value
        msg.anchor_id = anchor_id
        msg.waypoint_count = len(self.uwb_online_initialisation.remaining_waypoints)
        
        # Extract waypoint coordinates
        for waypoint in self.uwb_online_initialisation.remaining_waypoints:
    ...
    def _publish_anchor_positions(self):
        ...
        for anchor_id, anchor in self.unknown_anchors.items():
            ...
            if anchor_id not in self.uwb_online_initialisation.anchor_measurements_dictionary:
                continue
                
            measurements_dict = self.uwb_online_initialisation.anchor_measurements_dictionary[anchor_id]
            
            # Linear estimate (blue)
            if "estimator_rough_linear" in measurements_dict and len(measurements_dict["estimator_rough_linear"]) >= 3:
                linear_pos = measurements_dict["estimator_rough_linear"][:3]
                marker = self._create_anchor_marker(
                    linear_pos, 
                    marker_id, 
                    [0.0, 0.0, 1.0],  # Blue
            ...
            if "estimator_rough_non_linear" in measurements_dict and len(measurements_dict["estimator_rough_non_linear"]) >= 3 and (measurements_dict["status"] == "optimised_trajectory" or measurements_dict["status"] == "initialised"):
                nonlinear_pos = measurements_dict["estimator_rough_non_linear"][:3]
                marker = self._create_anchor_marker(
                    nonlinear_pos, 
                    marker_id, 
                    [0.8, 0.0, 0.8],  # Purple
            ...
            # Final estimate (red)
            if "estimator" in measurements_dict and len(measurements_dict["estimator"]) >= 3 and measurements_dict["status"] == "initialised":
                final_pos = measurements_dict["estimator"][:3]
                marker = self._create_anchor_marker(
                    final_pos, 
                    marker_id, 
                    [1.0, 0.0, 0.0],  # Red
    ...
    def _publish_optimised_waypoint_markers(self):
        ...
        if not hasattr(self.uwb_online_initialisation, 'current_optimal_waypoints') or not self.uwb_online_initialisation.current_optimal_waypoints:
            return
